...
class SCANCluster:
    ...
    def display(self):
        ...
        self.disp = np.zeros((800,400,3), dtype=np.uint8)
        # disp = cv2.cvtColor(disp, cv2.COLOR_BGR2HSV)
        ...
        # self.disp = cv2.rectangle(self.disp, (165, 530), (235, 670), yellow_color, -1)
        bus = cv2.imread('bus.png', cv2.IMREAD_COLOR)
        cv2.imshow('bus',bus)
        print(bus)
        h, w = bus.shape[:2]
        crop = self.disp[30:h+30, 30:w+30]
        cv2.copyTo(bus, crop)

        self.disp = cv2.line(self.disp, (5,0), (5,800), white_color, 2)
        self.disp = cv2.line(self.disp, (135,0), (135,800), white_color, 2)
        self.disp = cv2.line(self.disp, (265,0), (265,800), white_color, 2)
        self.disp = cv2.line(self.disp, (395,0), (395,800), white_color, 2)
        ...
        for i in self.cluster_list:
            ...
            self.disp = cv2.rectangle(self.disp, (200-i[1]-width, 600-i[0]-length), (200-i[1]+width, 600-i[0]+length), self.red_color, -1)

        cv2.imshow("display", self.disp)
        cv2.waitKey(1)
