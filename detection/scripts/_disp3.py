...
class USERDisplay:
    ...
    def draw(self):
        ...
        elif self.lane_color == 2 or self.lane_color == 3:
    ...
    def detect_obstacle(self):
        ...
        for i in self.cluster_list:
            ...
            if obstacle_y is not None and 360 > obstacle_y - width > 0 and 600 - obj_x - length > 0:
                if car_direction:
                    self.draw_logo(self.logo_car_front, obstacle_y - width, 600 - obj_x - length, self.obstacle)       
                else:
                    self.draw_logo(self.logo_car_back, obstacle_y - width, 600 - obj_x - length, self.obstacle)   
        ...
        # 내 차 그리기
        self.draw_logo(self.logo_bus, 200 - width, 600 - length, self.obstacle)
        # self.obstacle = cv2.rectangle(self.obstacle, (200 - width, 600 - length), (200 + width, 600 + length), yellow_color, -1)

    def draw_logo(self, logo, y, x, display):
        h, w = logo.shape[:2]
        roi = display[x:x+h, y:y+w]#배경이미지의 변경할(다음 로고 넣을) 영역
        ...
        display[x:x+h, y:y+w] = dst  #roi를 제자리에 넣음
    ...
    def detect_backward(self):
        if self.obstacle_BL:
            ...
            self.draw_logo(self.logoL, 75, 650, self.obstacle)

        if self.obstacle_BR:
            # print("BRBRBRB\n")

            # self.draw_logo(self.logoR, 790, 890, self.obstacle)
            self.draw_logo(self.logoR, 225, 650, self.obstacle)
