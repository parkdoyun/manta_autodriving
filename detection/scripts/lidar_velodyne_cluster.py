    ...
    def callback(self, msg):    
        ...
        if len(self.pc_np) == 0:
            ...
            for c in range(n_cluster):
                ...
                c_tmp_min = np.min(pc_xy[db==c, :], axis=0)
                c_tmp_max = np.max(pc_xy[db==c, :], axis=0)
                ...
                tmp_pose_min=Pose()
                tmp_pose_min.position.x = c_tmp_min.tolist()[0] +1
                tmp_pose_min.position.y = c_tmp_min.tolist()[1]

                tmp_pose_max=Pose()
                tmp_pose_max.position.x = c_tmp_max.tolist()[0] +1
                tmp_pose_max.position.y = c_tmp_max.tolist()[1]
                ...
                cluster_msg.poses.append(tmp_pose_min)
                cluster_msg.poses.append(tmp_pose_max)

                cluster_list.append({'id':c, 'dist': distance, 'ang': angle})
                cluster_list.append([tmp_pose.position.x, tmp_pose.position.y])
