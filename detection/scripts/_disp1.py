...
class USERDisplay:
    ...
    def LFA(self):
        ...
        cv2.line(self.img_bgrL, (540, 50), (540, 50), (0, 0, 255), 10)
        cv2.line(self.img_bgrR, (100, 50), (100, 50), (0, 0, 255), 10)

        # cv2.imshow('ll', self.img_bgrL)
        # cv2.imshow('Rr', self.img_bgrR)

        # cv2.imshow('white_laneL', white_laneL)
        # cv2.imshow('white_laneR', white_laneR)
        # cv2.imshow('yellow_laneL', yellow_laneL)
        ...
        if len(pointsL) >=2:
            ...
            if x2_L-x1_L:
                alpha = float((y2_L-y1_L)/(x2_L-x1_L))
                # if alpha < -1.9 or alpha > 1.9:
                if -0.6 < alpha < 0.6:
                    beta = y1_L - alpha*x1_L

                if alpha != 0:
                    self.targetL = (480-beta)/alpha
                    self.queueL.append(self.targetL)
                    if len(self.queueL) > 3:
                        self.queueL.pop(0)
        ...
        if len(pointsR) >=2:
            ...
            if x2_R-x1_R:
                alpha = float((y2_R-y1_R)/(x2_R-x1_R))
                # if alpha < -1.9 or alpha > 1.9:
                if -0.6 < alpha < 0.6:
                    beta = y1_R - alpha*x1_R

                if alpha != 0:
                    self.targetR = (480-beta)/alpha
                    self.queueR.append(self.targetR)
                    if len(self.queueR) > 3:
                        self.queueR.pop(0)
    ...
    def draw(self):
        ...
        self.obstacle = cv2.line(self.obstacle, (tL,0), (tL,800), black_color, 2)
        self.obstacle = cv2.line(self.obstacle, (tL-130,0), (tL-130,800), black_color, 2)
        self.obstacle = cv2.line(self.obstacle, (tR+130,0), (tR+130,800), black_color, 2)
        self.obstacle = cv2.line(self.obstacle, (tR,0), (tR,800), black_color, 2)
