...
class dijkstra_path_pub:
    def __init__(self):
        ...
        self.local_publisher = rospy.Publisher('/local_current_waypoint_update', String,
        ...
        self.way_point = [(100.843170221, 1154.87646303),
                          (110.504402215, 1273.46960267),
                          (107.631889398, 1367.67687807),
                          (70.5590134219, 1588.21667299),
                          (54.367576654, 1147.85204896),
                          (58.8529015133, 1913.37536439),
                          (199.031616265, 1356.89330873),
                          (48.0535431469, 1212.3765851)]
        # self.way_point = [(118.323280391, 1279.82800111), (135, 1410)]
        self.way_point_node = []
        for i in self.way_point:
            self.way_point_node.append(self.init_node(i[0], i[1]))

        # file에서 받은 거 읽기 (정류장의 노드 번호)
        # with open("/home/parkdoyun/route_info/route_station.txt", 'r') as file_data:
        #     line = None
        #     while True:
        #         line = file_data.readline().strip('\n')
        #         if line == '': break
        #         self.way_point_node.append(line)
        # print(self.way_point_node)
        #
        # self.student_station_dic = defaultdict(list)
        # # file 읽기 (해당 정류장에서 내리는 원생들)
        # with open("/home/parkdoyun/route_info/route_student.txt", 'r') as file_data:
        #     line = None
        #     while True:
        #         line = file_data.readline().strip('\n')
        #         if line == '': break
        ...
        self.cache = []
        self.path = []
        self.current_waypoint_before =0
        ...
        # path make
        self.path_make()
        self.global_path_pub.publish(self.global_node_path_msg)
        self.local_publisher.publish("1")
        # self.global_path_pub.publish(self.global_node_path[0])

        # self.mysql_file_path = '/home/parkdoyun/route_info/route.txt'  # mysql로 전송할 파일
        #
        # self.path_make()  # mysql에 보낼 경로 파일 만드는 함수
        #
        ...
        # # publish 두번 보내야 함
        # mysql_idx = 0
        # self.mysql_rate = rospy.Rate(30)
        # self.mysql_path = "/home/parkdoyun/route_info/route.txt"  # file path
        # while not rospy.is_shutdown():
        #     if mysql_idx == 4: break  # 두번 보내면 실행됨
        ...
        while not rospy.is_shutdown():
            ...
            if self.is_path == True and self.is_odom == True:
                ...
                self.current_waypoint_before = self.current_waypoint
                self.global_path_pub.publish(self.global_node_path_msg)
                self.local_publisher.publish("0")
                ...
                if self.is_look_forward_point:
                    ...
                    next_location = str(
                        self.nodes[self.way_point_node[(self.current_node + 1) % stop_len]].point[0]) + ',' + str(
                        self.nodes[self.way_point_node[(self.current_node + 1) % stop_len]].point[1])
                    ...
                    if tmp_dis >= 100:
                        ...
                        self.path_make()
                        self.local_publisher.publish("1")  # local_path_pub의 current_waypoint 갱신하라
                        rate.sleep()
                    ...
                    if tmp_dis < 5:  # 다음 경로 publish 하고 가리키는 현재 노드 갱신
                        ...
                        self.real_bus.header.frame_id = '/map'
                        self.path_make()
                        self.local_publisher.publish("1")  # 갱신하라
                        print(self.current_node)
                        self.next_stop = self.node_order[(self.current_node + 1) % stop_len]
                        rate.sleep()  # 바로 다음 경로로 넘어가는 것 방지
                        rate.sleep()

                    # else:  # 경로 이탈했다면 => 다음 노드로의 새로운 길 찾아서 이걸로 해
                    #     # 가장 가까운 노드 찾기
                    #     near_node_idx = self.init_node(self.current_postion.x, self.current_postion.y)
                    #     # 다음 정류장 노드까지의 A*star로 경로 찾고
                    #     tmp_result, tmp_path = self.global_planner.A_star(self.nodes[near_node_idx], self.nodes[
                    ...
                    #     self.global_path_pub.publish(tmp_new_back_path)
                    #     rate.sleep()
                    ...
                    #     # mysql_idx = 0
                    #     # while not rospy.is_shutdown():
                    #     #     if mysql_idx == 2: break  # 두번 보내면 실행됨
                    #     #     rospy.loginfo("[MYSQL] route write <= " + self.mysql_path)
                    #     #     mysql_publisher.publish(self.mysql_path)
                    #     #     self.mysql_rate.sleep()
                    #     #     mysql_idx += 1
    ...
    def path_make(self):
        ...
        self.global_node_path_msg = Path()
        self.global_node_path_msg.header.frame_id = '/map'
        ...
        for now in range(len(self.global_node_path)):
            self.global_node_path_msg.poses = self.global_node_path_msg.poses + self.global_node_path[now].poses
        # print("send_new_global")
    ...
    def calc_A_star_path_node(self):
        ...
        for i in range(len(self.path)):
            self.node_order.append((self.nodes[self.way_point_node[self.path[i]]].point[0],
                                    self.nodes[self.way_point_node[self.path[i]]].point[1]))  # node_idx로부터 x,y 구하기
